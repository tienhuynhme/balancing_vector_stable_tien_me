...
# Lưu lịch sử sau mỗi lần test
if "history" not in st.session_state:
    st.session_state.history = []

if "components" in st.session_state and "residual_moment" in st.session_state:
    if len(st.session_state.components) > 0:
        st.session_state.history.append({
            ...
            "Số vector sinh ra": len(st.session_state.components),
            "Sai số còn lại": round(st.session_state.residual_moment, 4)
        })

# Export file CSV
if st.session_state.history:
    if st.button("📤 Tải tất cả kết quả test hôm nay"):
        df_export = pd.DataFrame(st.session_state.history)
        csv_buffer = io.StringIO()
        df_export.to_csv(csv_buffer, index=False)
        st.download_button("📥 Nhấn để tải file CSV", csv_buffer.getvalue(), file_name="ket_qua_test_hom_nay.csv", mime="text/csv")
